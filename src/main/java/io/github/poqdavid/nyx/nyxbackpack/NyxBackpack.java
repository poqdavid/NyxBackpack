...
@Plugin(id = PluginData.id, name = PluginData.name, version = PluginData.version, description = PluginData.description, url = PluginData.url, authors = {PluginData.author1}, dependencies = {@Dependency(id = "nyxcore", version = "1.0", optional = false)})
public class NyxBackpack {
    ...
    public NyxBackpack(Metrics.Factory metricsFactory, @ConfigDir(sharedRoot = true) Path path, Logger logger, PluginContainer container) {
        ...
        this.logger = NyxCore.getInstance().getLogger("BP");
